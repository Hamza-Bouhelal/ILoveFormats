...
export const fileValidationMiddleware = (
  ...
  try {
    ...
    const expectedExtension = req.format;

    if (fileExtension?.toLocaleLowerCase() !== expectedExtension) {
      return res.status(400).json({
        error: `Invalid file extension. Expected: ${expectedExtension}`,
      });
    }
    ...
    file.mv(`${dir}/${file.name}`, (err) => {
      if (err) {
        ...
        if (!fs.existsSync(`${dir}/${file.name}`)) {
          ...
          return res
            .status(500)
            .json({ error: "Error while processing file." });
        }
